    ...
    mymap.on('drag', function() {
        ...
        // console.log(latLimit);
        // console.log(lonLimit);
    });
    ...
    populatePanel(uniqueImagesPanel, restAllImages);
    ...
    function insertFinalImages(restAllImages, direction, toggleDirection, opacity) {
        $.each(restAllImages, (index, imageFullPath) => {
            ...
            imagesList.push(
                addImage(imageFullPath, direction, parent, imageData.width, imageData.height, toggleDirection, opacity));
    ...
    function populatePanel(uniqueImagesPanel, restAllImages) {
        let direction = 2; // starting direction right
        let toggleDirection = 1; // switch between adding to the right and adding to the left

        insertFinalImages(uniqueImagesPanel, direction, toggleDirection, 1);
        insertFinalImages(restAllImages, direction, toggleDirection, 0.2);
    ...
    function calculateImageBounds(variableOriginX, variableOriginY, imageWidth, imageHeight) {
    ...
    function addImage(imagePath, direction, parent, imageWidth, imageHeight, toggleDirection, opacity) {
        // direction:
        // 1 = UP
        // -1 = DOWN
        // 2 = RIGHT
        ...
        if (parent === null || parent === undefined) { // this is the first image that has to be placed in the center
            let imageBounds = calculateImageBounds(staticOriginX, staticOriginY, imageWidth, imageHeight);
        ...
        let imageBounds = calculateImageBounds(variableOriginX, variableOriginY, imageWidth, imageHeight);
        ...
        while (overlay || margin > 0) {
            ...
            imageBounds = calculateImageBounds(variableOriginX, variableOriginY, imageWidth, imageHeight);
